...
export function MediaPreview({
    ...
    return (
      ...
      >
        <div className="text-center text-muted p-4">
          ...
          <p className="text-xs font-semibold text-destructive/80 mb-1">Invalid Image URL</p>
          <p className="text-[10px]">The link provided is not a direct image file or is blocking access.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`${aspectClasses[aspectRatio]} rounded-lg overflow-hidden relative ${className}`}>
      ...
      <img
        src={useProxy ? `https://images.weserv.nl/?url=${encodeURIComponent(url)}&w=800` : url}
        alt={alt}
